...
func (s *SuiteBase) TestUpsertLink(c *gc.C) {
	...
	// Update existing link with a newer timestamp and different RTL
	...
	c.Assert(err, gc.IsNil)
	c.Assert(existing.ID, gc.Equals, original.ID, gc.Commentf("last accessed timestamp was not updated"))
	...
	c.Assert(err, gc.IsNil)
	c.Assert(dup.ID, gc.Not(gc.Equals), uuid.Nil, gc.Commentf("expectedd a linkID to be assigned to the new link"))
}
...
func (s *SuiteBase) TestLinkIteratorTimeFilter(c *gc.C) {
	...
	for i, t := range linkInsertTimes {
		c.Logf("fetching links created beforee edge %d", i)
...
func (s *SuiteBase) assertIteratedLinkIDsMatch(c *gc.C, updatedBefore time.Time, exp []uuid.UUID) {
	...
	sort.Slice(got, func(l, r int) bool {
		return got[l].String() < got[r].String()
	})
	sort.Slice(exp, func(l, r int) bool {
		return exp[l].String() < exp[r].String()
	})
