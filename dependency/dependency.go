...
type API interface {
	// ListDependencies returns the list of direct dependency IDs for a particular project ID or an error if a
	// non-project ID argument is provided.
...
// AllDependencies returns the unique list of direct and transitive dependencies for the project with the specified ID.
...
// scanProjectDependencies adds each item in depList to the list of direct and transitive dependencies maintained by
// ctx. In addition, it checks the type of each dependency in depList and enqueues any encountered sub-projects for
// recursive scanning.
