...
func (s *PingerSuite) SetUpTest(_ *check.C) {
	s.origPath = os.Getenv("PATH")
}

func (s *PingerSuite) TearDownTest(_ *check.C) {
...
func mockCmdOutput(c *check.C, cmdName, output string, exitCode int) {
	...
	fakeBin, err := os.OpenFile(pathToFakeBin, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0777)
	c.Assert(err, check.IsNil)
