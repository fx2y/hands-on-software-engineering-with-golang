...
// Prompter is implemented by objects that display a CAPTCHA image to the user, ask them to type their contents and
// return back their response.
type Prompter interface {
	Prompt(img image.Image) string
}

// ChallengeUser requests a challenge from c and prompts the user for an answer using p. If the user's answer matches
// the challenge then ChallengeUser returns true. All comparisons are performed using constant-time checks to prevent
// information leaks.
